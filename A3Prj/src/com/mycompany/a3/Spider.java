...
public class Spider extends Moveable  implements IDrawable {
	...
	private static final int SPIDER_COLOR = ColorUtil.rgb(0, 0, 0); // Black
	
	public Spider() {
		
		super(generateRandomSize(), generateRandomLocation(), SPIDER_COLOR, rand.nextInt(360), 5 + rand.nextInt(11));
	...
	public void setColor(int color) {
		// set to black no matter what color is passed in.
		super.setColor(ColorUtil.BLACK);
	}	
	
	// i think this is gonna be needed later
	public void updateHeadingRandomly() {
		int headingChange = (new Random().nextInt(11)) - 5; // +- 5 degrees
	...
	public void draw(Graphics g, com.codename1.ui.geom.Point pCmpRelPrnt) {
		...
		int spiderSize = this.getSize();
		
		int x = pCmpRelPrnt.getX() + (int)this.getLocation().getX() - (spiderSize / 2);
		int y = pCmpRelPrnt.getX() + (int)this.getLocation().getY() - (spiderSize / 2);
		
		g.setColor(this.getColor());
		g.drawRect(x,  y,  spiderSize,  spiderSize);
